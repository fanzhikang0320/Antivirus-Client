      ...
      <div class="main clearfix">
        <div class="main-left">
          <div class="product">
            ...
            <a :href="item.product.link" target="_blank" class="btn">Visit Antivirus</a>
          </div>
          ...
          <div id="PAC">
            ...
            <a :href="item.product.link" target="_blank" class="btn">Visit {{item.product.name}}</a>
          </div>
...
export default {
  ...
  watch: {
    $route: {
      handler() {
        this.getReview(this.$route.query.productId);
  ...
  methods: {
    getReview(productId) {
      this.axios.post('/reviewApi/selectAllInfo',{productId: productId})
        .then(result => {
